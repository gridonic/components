...
export class DebuggerService extends TonicService {
    ...
    set status(status) {
        ...
        // If localStorage is available, write the current status into it. Note that a fake store is used if it is not available
     ...
     */
    log(message: string, options:any = DebuggerService.CORE) {
     ...
     * @param {object|number} options Optional options for changing the way of logging.
     */
    warn(message: string, options = DebuggerService.WARNING) {
     ...
     * @param {object|number} options Optional options for changing the way of logging.
     */
    error(message: string, options = DebuggerService.ERROR) {
        this.log(message, options);
    }
